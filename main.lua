-- Premium Multi-Tool GUI by KDML
-- Modern Neon Platform & Infinite Baseplate
...
-- ===== NEON PLATFORM FUNCTIONS =====
local function createNeonEdges(parent)
    for _, e in pairs(neonEdges) do
    ...
    local function makeEdge(p1, p2)
        ...
        edge.CFrame = CFrame.new(mid, mid + (p2 - p1).Unit)
        edge.Parent = parent

        pcall(function()
            local light = Instance.new("PointLight")
            light.Color = NEON_CONFIG.Color
            light.Range = 8
            light.Brightness = 2
            light.Parent = edge
        end)
...
local function createNeonPlatform()
    if neonPlatform and neonPlatform.Parent then 
        neonPlatform:Destroy() 
    end
    for _, e in pairs(neonEdges) do 
        if e and e.Parent then e:Destroy() end 
    end
    ...
    neonPlatform.TopSurface = Enum.SurfaceType.Smooth
    neonPlatform.BottomSurface = Enum.SurfaceType.Smooth
    ...
    pcall(function()
        ...
        decal.Texture = NEON_CONFIG.ImageId
        decal.Face = Enum.NormalId.Top
        decal.Transparency = 0.2
        decal.Parent = neonPlatform
    end)
...
local function destroyNeonPlatform()
    if neonPlatform then
        pcall(function() neonPlatform:Destroy() end)
        neonPlatform = nil
    end
    for _, e in pairs(neonEdges) do
...
local function destroyBasePlatform()
    if basePlatform then
        basePlatform:Destroy()
        basePlatform = nil
    end
...
RunService.Heartbeat:Connect(function(dt)
    if neonEnabled and neonPlatform and hrp and humanoid then
        ...
        targetPos = Vector3.new(
            targetPos.X,
            math.max(NEON_CONFIG.MinY, math.min(NEON_CONFIG.MaxY, targetPos.Y)),
            targetPos.Z
        )
...
local neonToggleCorner = Instance.new("UICorner")
neonToggleCorner.CornerRadius = UDim.new(0,6)
neonToggleCorner.Parent = neonToggle
...
local baseToggleCorner = Instance.new("UICorner")
baseToggleCorner.CornerRadius = UDim.new(0,6)
baseToggleCorner.Parent = baseToggle
...
addHoverEffect(closeBtn, Color3.fromRGB(255,60,60), Color3.fromRGB(255,80,80))
addHoverEffect(neonToggle, Color3.fromRGB(60,60,65), Color3.fromRGB(80,80,80))
addHoverEffect(baseToggle, Color3.fromRGB(60,60,65), Color3.fromRGB(80,80,80))

-- ===== STATUS UPDATE =====
local function updateNeonStatus(active)
    if active then
        neonStatus.Text = "Status: ACTIVE ✓"
        neonStatus.TextColor3 = Color3.fromRGB(60,255,60)
        neonToggle.Text = "DEACTIVATE"
        neonToggle.BackgroundColor3 = Color3.fromRGB(255,60,60)
    else
        neonStatus.Text = "Status: OFFLINE"
        neonStatus.TextColor3 = Color3.fromRGB(150,150,150)
        neonToggle.Text = "ACTIVATE"
...
local function updateBaseStatus(active)
    if active then
        baseStatus.Text = "Status: ACTIVE ✓"
        baseStatus.TextColor3 = Color3.fromRGB(60,255,60)
        baseToggle.Text = "DEACTIVATE"
        baseToggle.BackgroundColor3 = Color3.fromRGB(255,60,60)
    else
        baseStatus.Text = "Status: OFFLINE"
        baseStatus.TextColor3 = Color3.fromRGB(150,150,150)
        baseToggle.Text = "ACTIVATE"
